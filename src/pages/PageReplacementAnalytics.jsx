      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_300px] gap-[20px] items-start">
          ...
          {!referenceString && (
            <div className="p-12 text-center text-text-muted card border-dashed border-border-muted flex flex-col items-center justify-center">
              ...
              <p className="text-sm text-text-muted mt-2 text-center max-w-sm">
                Add a reference string in the Simulator first to view analytics.
              </p>
            </div>
          )}

          {/* Tab Content */}
          {referenceString && activeTab === 'belady' && (
        ...
        <div className="flex flex-col gap-[20px] w-full">
          <div className="card p-4 space-y-2">
            <h3 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-2">View Analytics</h3>
            <button
              onClick={() => setActiveTab('belady')}
              className={`w-full text-left px-4 py-3 text-sm font-medium rounded-[5px] transition-colors ${
                activeTab === 'belady' ? 'bg-accent/20 text-accent border border-accent/50' : 'bg-surface text-text-muted hover:bg-elevated'
              }`}
            >
              Belady's Anomaly
            </button>
            <button
              ...
              className={`w-full text-left px-4 py-3 text-sm font-medium rounded-[5px] transition-colors ${
                activeTab === 'heatmap' ? 'bg-accent/20 text-accent border border-accent/50' : 'bg-surface text-text-muted hover:bg-elevated'
              }`}
            >
              LRU Fault Heatmap
            </button>
