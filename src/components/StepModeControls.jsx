  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      timer = setInterval(() => {
        ...
        if (state.stepIndex < totalBlocks) {
          state.stepForward()
        } else {
          setIsPlaying(false)
        }
    ...
    <div className="card p-4 space-y-4">
      ...
      <div className="flex items-center gap-2 justify-center">
        ...
        <button
          ...
          className={`btn-secondary p-2 ${isPlaying ? 'text-green' : ''}`}
      ...
      <div className="grid grid-cols-2 gap-3">
        {/* Running process */}
        <div className="bg-elevated rounded-[5px] p-3">
          <p className="text-xs text-text-muted mb-1">Running Now</p>
          {currentBlock ? (
            <div className="flex items-center gap-2">
              <div
                className="w-3 h-3 rounded-full animate-pulse"
                style={{ backgroundColor: getPIDColor(currentBlock.pid, processes) }}
              />
              <span className="font-mono font-bold text-text-primary">{currentBlock.pid}</span>
        ...
        {/* Ready queue */}
        <div className="bg-elevated rounded-[5px] p-3">
          <p className="text-xs text-text-muted mb-1">Ready Queue ({readyQueue.length})</p>
            ...
            {readyQueue.length === 0 ? (
              ...
              readyQueue.map((p) => (
                <span
                  ...
                  style={{
                    borderColor: getPIDColor(p.id, processes) + '60',
                    color: getPIDColor(p.id, processes),
                    backgroundColor: getPIDColor(p.id, processes) + '15',
                  }}
                >
                  {p.id}
                </span>
